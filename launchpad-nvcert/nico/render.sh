...
#
# Usage:
#   export BMC_PASSWORD='<the tray/DPU BMC password>'   # from Vault, do NOT paste into a file
#   ./render.sh
#   # -> writes expected_machines.nvcert.json  (git-ignored)
#
set -euo pipefail
_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
: "${BMC_PASSWORD:?set BMC_PASSWORD (get it from Vault / the team) before rendering}"

TEMPLATE="${_DIR}/expected_machines.nvcert.template.json"
OUT="${_DIR}/expected_machines.nvcert.json"
