...
| unbound | 172.16.2.42 | | nico-ssh-console-rs | .49 |
...
```
- DHCP relay is a **self-hosted `isc-dhcp-relay` on cp-1 (giaddr `172.16.2.11`) → 172.16.2.41** — see
  [DHCP-RELAY.md](../DHCP-RELAY.md). This is MANDATORY — Kea rejects non-relayed packets.
...
- A tray DHCPDISCOVER reaches nico-dhcp (cp-1 relay giaddr 172.16.2.11 → launchpad-mgmt segment) and PXE-boots aarch64.
- Trays reach `Ready`.
...
- DHCP relay: self-hosted `isc-dhcp-relay` on cp-1 (giaddr 172.16.2.11 → kea VIP .41); see DHCP-RELAY.md. (Core-switch ip-helper is the optional long-term home — no networking-team dependency for bring-up.)
...
## Next phases
After Core: continue with **REST → register site → Flow → RMS → rack config → ssh-console-rs**. See
[README §0.1](../README.md) for the ordering + per-layer docs (RMS-ENABLEMENT.md, RACK-CONFIG.md,
FLOW-FIXES.md, SSH-CONSOLE.md).
