#!/usr/bin/env bash
# Stand up a durable `admincli` pod for the nvcert NICo cluster (Core-only, no REST/OIDC).
...
#
# Run:  KUBECONFIG=/path/to/nvcert/kubeconfig bash admincli-setup.sh
set -euo pipefail

NS=nico-system
IMAGE="nvcr.io/0837451325059433/carbide-dev/nvmetal-carbide:v2.0.0-pr-503-g49a48a69d"
: "${KUBECONFIG:?export KUBECONFIG to the nvcert kubeconfig}"
...
V "vault write -format=json nicoca/issue/admin-cli common_name=admin-cli ttl=8760h" > /tmp/nvcert-cli.json
python3 - <<'PY'
import json
d=json.load(open('/tmp/nvcert-cli.json'))['data']
open('/tmp/nvcert-cli.crt','w').write(d['certificate']+"\n")
open('/tmp/nvcert-cli.key','w').write(d['private_key']+"\n")
open('/tmp/nvcert-cli-ca.crt','w').write(d['issuing_ca']+"\n")
PY
openssl x509 -in /tmp/nvcert-cli.crt -noout -subject -enddate

echo "== 4. Store as secret admincli-cert =="
kubectl create secret generic admincli-cert -n "$NS" \
  --from-file=tls.crt=/tmp/nvcert-cli.crt --from-file=tls.key=/tmp/nvcert-cli.key --from-file=ca.crt=/tmp/nvcert-cli-ca.crt \
  --dry-run=client -o yaml | kubectl apply -f -
rm -f /tmp/nvcert-cli.json /tmp/nvcert-cli.crt /tmp/nvcert-cli.key /tmp/nvcert-cli-ca.crt
